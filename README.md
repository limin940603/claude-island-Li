...
- **设置控制台(Windows)**:图形化设置窗口——总开关(暂停弹窗)、开机自启、静默、音量、岛体不透明度、暗/亮双主题、按状态静音、**每状态可换提示音(自带 4 个 + 系统 `C:\Windows\Media` 精选,带 ▶ 试听)**、**系统硬件监控(空闲 30 秒后岛上显示 CPU/内存,新事件立即让位)**、**贴边隐藏(空闲缩进顶边留细条,悬停/新事件弹出)**、今日统计 + 近 7 日事件趋势。托盘右键「设置控制台…」或双击托盘图标打开,改动即时生效。
...
- `edgeHide`(Windows):贴边隐藏——空闲 30 秒岛自动滑进屏幕顶边只留细条;鼠标悬停细条或新事件到来立即滑出(触发即弹出);岛被拖离顶边(Top>60)时不吸附;滑动期间不写 pos.json。
